...
- Without an execution choice, Codex GPT-Image-2 is proposed; an explicit user
  model choice wins.
- The flow stops at saved Preview for request approval, inspects generated
  output, waits for output acceptance, then imports with `--select` when the
  accepted intent is selection.
