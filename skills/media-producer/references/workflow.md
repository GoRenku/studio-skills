...
4. For Codex built-in image generation, use the system `$imagegen` workflow, save the selected output inside the project as a staging project-relative file under `generated/media/`, inspect it, and import it without `--receipt`.
...
2. Save the selected generated image as a staging source under
   `generated/media/<descriptive-file>.png` inside the Renku project. This path
   is the import source, not the durable asset location.
3. Import the staging file with the same purpose-specific `renku media import`
   command used for Renku-managed outputs. Do not manually copy files into
   canonical folders such as `cast/`, `locations/`, `visual-language/`, or
   `shotlist/`; Core owns those folders during import.
4. Omit `--receipt`. Never fabricate a Renku receipt for Codex-generated or
   manually supplied media.
