...
7. Run with `--simulate` for dry checks or with the cost approval token for paid generation.
8. Inspect finished media before import. For location environment sheets, inspect the composite, use vision to identify the four scenic view blocks, crop only those four blocks, and inspect the four slices.
9. Import finished media with `renku media import`.
