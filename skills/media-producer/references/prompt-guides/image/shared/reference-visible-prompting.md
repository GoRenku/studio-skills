...
Read current domain context and the selected provider Skill before choosing an
endpoint. Use one exact model from that Skill's supported-model index.
...
target the registered source Asset. Resolve the exact source file first. A
Fal.ai GPT Image 2 edit review document has this provider-native shape:
...
```json
{
  ...
  "request": {
    "prompt": "Edit this exact source image. Preserve the accepted regions, style, layout, lighting, and material detail. Change only the requested area: [specific user correction].",
    "image_urls": [
      { "$file": "tmp/scratch/source.png", "mimeType": "image/png" }
    ]
  }
}
```
...
Choose an indexed provider model and inspect that operation's current provider
facts. Good prompts name concrete materials, light, skin texture, cloth
weight, age read, posture, lens/finish, exact layout, and the provider-visible
role of every attached reference.

Respect the selected provider guide's media cardinality. If it exposes no media
