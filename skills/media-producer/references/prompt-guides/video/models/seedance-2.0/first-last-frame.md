...
exposes `image_url` and `end_image_url`, and both endpoints are binding. Put the
exact First Frame local marker in `image_url` and the exact Last Frame local
marker in `end_image_url`.
