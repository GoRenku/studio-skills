...
references matter. Author `generate_audio` only when the selected provider guide
exposes it. Put an exact audio local-file marker in `audio_urls` only for a
selected reference-to-video route that exposes that native field.
