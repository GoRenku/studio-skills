...
- selected direct endpoint is reference-to-video and its provider guide exposes
  `image_urls`;
- an exact storyboard/reference image is placed in the native `image_urls`
  array;
...
2. Validate the exact temporary review document, show Preview when required,
   and use its native request as the field-assignment and token-order source of
   truth.
...
Map references by the final provider-native request. For Seedance
...
`@Image2`, `@Image3`, or later image tokens only when the final native request
...
- the prompt does not contradict storyboard, current context facts, or user
  corrections;
- fields absent from the selected provider guide are not authored, and key
