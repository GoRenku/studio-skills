...
required `image_url` and an opening image anchors the video. Put the exact First
Frame local-file marker in `image_url`. If the route also exposes
`end_image_url`, include an exact Last Frame only when it is binding.

## Prompt Contract

- Use an opening-image token only if the selected endpoint documentation
  establishes one. Otherwise describe the opening
