...
Read `index.md` first. Use this only for `minimax/h3/image-to-video`. Put the
binding opening-frame local marker in `image_url`. Put a binding final-frame
marker in `end_image_url` only when the selected input mode is
