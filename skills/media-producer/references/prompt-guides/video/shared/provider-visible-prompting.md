...
provider/model endpoint from its provider Skill.
...
declared by the selected provider guide.
...
fields that document modality-local mentions, map the final native array order.
For example:
