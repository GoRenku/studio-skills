...
1. Read the exact Shot Plan and its current Scene, Shot, Cast, Location, Prop,
   dialogue, and accepted media context.
2. Validate the exact temporary review document, show Preview when required,
   and use its native request plus provider adapter as the assignment and
   mention-order source of truth.
3. Inspect the actual storyboard/reference image, not only its title or
   thumbnail.
4. Read the earlier creative storyboard brief if available.

Do not infer tokens from filenames, memory, card order, visible thumbnails, or
earlier requests.

Map references by final modality-local request order. In the templates below,
`<IMAGE_1>`, `<VIDEO_1>`, and `<AUDIO_1>` are placeholders. Replace them with
the exact mentions supplied by the selected provider adapter.
...
`<IMAGE_2>`, `<IMAGE_3>`, or later image tokens only when the final native request
actually includes them and they supply a necessary role that the storyboard does
not already carry.

If the native request unexpectedly contains redundant upstream
location/look/character images for a realistic storyboard, stop before prompt
confirmation and revise the final input selection. Do not try to rescue
...
```text
<IMAGE_1> is the storyboard blueprint for this video. Read it as ordered video
...
Adjust mention and panel count to match the final request and visible image.
...
timing.
<IMAGE_2> is only [location / character / look / prop] continuity. Do not use it
as an alternate storyboard, first frame, page layout, or different geography.
<AUDIO_1> is the [narrator voice / ambience / sound-character] reference. Use it
...
references unless the final provider request truly includes them and the
...
- `NEGATIVE CONSTRAINTS`: put critical exclusions in the main prompt unless
  the selected operation's current provider facts expose another deliberate
  place for them.
...
tracks. Assign an exact audio file through the provider adapter and do not omit
its resolved mention when narration, dialogue, voice character, ambience, or
sound matters.

- Use `<AUDIO_N>` as narrator voice/style, speaker character, ambience, or
...
Before paid execution, compare the final prompt against hard constraints from:

1. current domain facts and Shot data;
2. user corrections;
3. storyboard brief, if available;
...
- the prompt does not contradict storyboard, current context facts, or user
  corrections;
- fields absent from the provider adapter or live schema are not authored, and
  key negative constraints are written into the main prompt.
...
- Fix: scope those references narrowly. Location Sheet means place continuity
  ...
  selected provider route explicitly receives it as one.
...
- Cause: the prompt includes narration/dialogue but does not name the supplied
  `<AUDIO_N>` token and its voice or sound role.
