...
- Codex built-in image generation is allowed only for image purposes. It bypasses Renku generation records by design, but it must not bypass Renku project metadata: stage the selected file under `tmp/media/` and attach it through `renku media import` without `--receipt`.
