...
If a command prints `CLI026`, the media import or mutation has already succeeded but Studio was not notified. Do not rerun non-idempotent imports just to notify Studio; that can duplicate assets. Read back durable state, then use a separate notification/recovery step with local network permission if available, or tell the user Studio needs a refresh.
...
3. Choose the execution path:
   - Use **Codex built-in image generation** when the user asks to use Codex, `$imagegen`, built-in image generation, GPT-Image 2 through Codex, or to avoid app-side/fal.ai generation costs for an image purpose. This includes `scene.storyboard-sheet`; do not route storyboard sheets to fal.ai solely because they are storyboard sheets.
